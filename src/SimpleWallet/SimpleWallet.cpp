...
const command_line::arg_descriptor<uint64_t> arg_DEFAULT_FEE  = {"DEFAULT_FEE", "Default fee", CryptoNote::parameters::MINIMUM_FEE};
...
std::string tryToOpenWalletOrLoadKeysOrThrow(LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password) {
  ...
  if (walletExists) {
    ...
    if (initError) { //bad password, or legacy format
      ...
      logger(WARNING, BRIGHT_RED) << "You can export your keys in the GUI wallet and use the 'Import' option here to import them and create a new wallet file.";
      if (keysExists) {
        std::stringstream ss;
